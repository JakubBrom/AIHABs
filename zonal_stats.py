...
def zonal_stats(vector_path, raster_path, method='median',
    ...
    while feat is not None:
        ...
        if not global_src_extent:
            ...
            try:
                src_array = rb.ReadAsArray(*src_offset).astype(
                    np.float32)
            except:
                src_array = None
        ...
        if method == 'median':
            ...
            feature_stats = {'fid': int(feat.GetFID()),
                             'sum': float(np.nansum(masked))}
        elif method == 'count':
            feature_stats = {'fid': int(feat.GetFID()), 'count': float(
                np.count_nonzero(~np.isnan(masked)))}
        elif method == 'mean':
            feature_stats = {'fid': int(feat.GetFID()),
